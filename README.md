...
src/
  ...
  control_store.py     面板可写的运行时开关(data/controls.json):是否允许开新仓、币种黑名单;主程序只读,面板是唯一写入方
...
可操作的两个开关(通过 `control_store.py` 写入 `data/controls.json`,主程序每次开仓检查都会重新从磁盘读取,不用重启即可生效):
...
面板本身**不提供下单或平仓的入口**,这两个开关都只影响"要不要评估新信号",不会主动操作任何已有仓位。
