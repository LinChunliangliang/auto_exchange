...
- **但持仓超过 `PROFIT_LOCK_AFTER_SECONDS` 且当前有盈利(哪怕没到止盈线)会锁定收益提前平仓**:持仓太久说明预期的快速突破大概率已经落空,继续拖着只是在赌反转不会发生;只在有盈利时触发,不影响亏损/持平的仓位
...
| `PROFIT_LOCK_AFTER_SECONDS` | 持仓超过这个秒数,只要当前有盈利(哪怕没到止盈线)就平仓锁定收益;亏损/持平的仓位不受影响 |
